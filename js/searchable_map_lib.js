    ...
    if (!SearchableMapLib.map) {
      ...
      legend.onAdd = function (map) {
        var div = L.DomUtil.create('div', 'info legend');
         var statuses = [
            { label: "Planned", icon: "marker-icon-2x-grey.png" },
            { label: "Dismantled", icon: "marker-icon-2x-red.png" },
            { label: "Production", icon: "marker-icon-2x-yellow.png" },
            { label: "Aproved", icon: "marker-icon-2x-blue.png" },
            { label: "Constuction", icon: "marker-icon-2x-green.png" },
            { label: "Test site", icon: "marker-icon-2x-violet.png" }
          ];

        div.innerHTML += '<h4 style="font-size:13px;">Status</h4>';
...
legend.addTo(SearchableMapLib.map);
  ...
  createSQL: function() {
    ...
    // -----end status filter-----
    // -----custom power filter-----
    // var maxPower = parseFloat($("#power-range").val());

    // if (!isNaN(maxPower)) {
    //   SearchableMapLib.currentResults.features = $.grep(SearchableMapLib.currentResults.features, function(r) {
    //     var power = parseFloat(r.properties["POWER_MW"]);
    //       return isNaN(power) || power <= maxPower;
    //   });
    // }
    // -----custom power filter-----
    ...
    SearchableMapLib.currentResultsLayer = L.geoJSON(SearchableMapLib.currentResults, {
        pointToLayer: function (feature, latlng) {
          return L.marker(latlng, {icon: SearchableMapLib.getIcon(feature.properties["STATUS"])} );
        },
  ...
  getIcon: function(STATUS){
    if (STATUS == "Planned") return greyIcon;
    if (STATUS == "Dismantled") return redIcon;
    if (STATUS == "Production") return yellowIcon;
    if (STATUS == "Aproved") return blueIcon;
    if (STATUS == "Constuction") return greenIcon;
    if (STATUS == "Test site") return violetIcon;
    return blueIcon;
  },
